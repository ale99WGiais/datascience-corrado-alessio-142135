...
wordCloud = function(theme_name = NA){
  ...
  data = rbind(
    df_words_tfidf %>% filter(is.na(theme_name) | theme==theme_name) %>% top_n(70) %>% select(tf_idf, label=word),
    df_bigram_tfidf %>% filter(is.na(theme_name) | theme==theme_name) %>% top_n(50) %>% select(tf_idf, label=bigram)
  ) %>% arrange(desc(tf_idf))
...
data = df_words_summary %>% filter(theme == "chemical colorants") %>% top_n(200, n)
